...
def decode_geometry(byte_list):
    """Convert the integer list to bytes."""

    try:
        geom_bytes = bytes(byte_list)
        # Use Shapely to decode the WKB format
        geometry_object = loads(geom_bytes)
    except Exception:
        float_list = []
        geom_list = []
        # Valid data starts at 40 bytes
        for i in range(40, len(byte_list), 8):
            float_list.append(
                struct.unpack('<d', byte_list[i:i+8])[0]
            )
        for i in range(0, len(float_list), 2):
            geom_list.append((float_list[i], float_list[i + 1]))
        geometry_object = Polygon(geom_list)
